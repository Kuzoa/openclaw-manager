...
/// Uses EnvironmentCache to avoid repeated npm config get prefix calls.
///
/// Detection phases:
...
pub fn get_openclaw_path() -> Option<String> {
    // Use the environment cache for lazy loading and caching
    super::cache::ENVIRONMENT_CACHE.get_openclaw_path()
}
...
pub fn run_openclaw(args: &[&str]) -> Result<String, String> {
    ...
    let openclaw_path = get_openclaw_path().ok_or_else(|| {
        debug!("[Shell] Cannot find openclaw command (OpenClaw not installed)");
...
fn get_gateway_token_from_config() -> String {
    ...
    if let Some(token) = existing_token {
        ...
        let needs_update = config
            ...
            != Some(true);
        if needs_update {
            debug!("[Shell] Setting gateway.controlUi.allowInsecureAuth = true");
        ...
        debug!("[Shell] Using gateway token from config");
        return token;
    }
